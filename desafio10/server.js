...
import ContenedorCarritoMongo from './contenedores/contenedorCarritoMongo.js'
let listaDeProductos = new ContenedorProductosMongo
//let productosEnCarrito = new ContenedorCarritoFirebase
let productosEnCarrito = new ContenedorCarritoMongo
...
routerCarrito.post('/:id/productos', async (req, res) => {
    ...
    const productoParaAgregarACarrito = await listaDeProductos.getById(productId)
    res.json(await productosEnCarrito.saveProductInCart(id, productId, JSON.stringify(productoParaAgregarACarrito, null, 2)))
})
